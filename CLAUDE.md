...
uv run pytest                    # run all 1091 tests (~40s)
...
- **Version:** `0.8.2` in `pyproject.toml`, running on the local fleet (soaking); **not yet published** to PyPI/Homebrew (0.8.0/0.8.1 were soaked but superseded — the accumulated 0.8.x work publishes together as `0.8.2`; latest published is still `0.7.0`). **0.8.2** fixes a fire-and-forget trace-write GC bug (failed-request traces were being garbage-collected before persisting, so the dashboard success rate was blind to failures — `_create_logged_task` now holds strong task refs; exhausted-retry now records a terminal `failed` trace) and corrects the MLX idle-server-churn diagnosis (external OS memory-pressure kill, not a supervisor health kill — mitigated by dropping the unused Qwen3-Coder-Next server). Since 0.7.0: **0.8.0** added distributed MLX inference (`mlx.launch` ring/jaccl, `FLEET_NODE_MLX_SERVERS` as the single MLX config surface, removing legacy single-server env vars) + the `0.0.0.0` health-poll fix + MLX port-release-wait. **0.8.1** adds client-ergonomics from agent feedback: canonical `X-Fleet-*` headers (`X-Fleet-Served-Model`/`X-Fleet-Requested-Model`/boolean `X-Fleet-Fallback`, retiring `X-Fleet-Model`) via a shared `fleet_headers()` builder; per-request strict mode (`X-Fleet-No-Fallback` header overriding global `FLEET_VRAM_FALLBACK`); queue-full 503 no longer retried (→ `429`); `GET /fleet/limits` + `POST/DELETE /fleet/pin`; and a per-client concurrency cap (`FLEET_CLIENT_MAX_IN_FLIGHT`, default 0=off). Also: **`mlx:` models are now reachable via the OpenAI `/v1/chat/completions` endpoint** (previously Anthropic-only) so OpenAI-only clients get the fast MLX backend instead of a slow Ollama fallback; `/v1/models` advertises healthy `mlx:` models; and MLX **reasoning-model output is surfaced** (the `reasoning` field — GLM-4.7-Flash etc. — was being dropped, returning empty responses). Also: **`POST /fleet/pin` gains a `wait` mode** (`{"wait": true, "timeout_s": 30}`) that blocks until the router's routing view confirms the model resident (`ready`/`ready_after_ms`) so a pin-then-scan client doesn't race the ~5s heartbeat into fallback; `mlx:` pins short-circuit as no-ops; fixes a latent pin-persistence race. See `docs/plans/client-ergonomics-from-agent-feedback.md` + `docs/plans/fleet-pin-readiness.md` + `docs/plans/distributed-mlx-inference.md`. **When updating skills:** `grep -rn "1057 tests\|1061 tests\|1072 tests\|1074 tests\|36 checks" skills/`.
...
- **Ollama:** **`0.32.1`** (upgraded from `0.24.0` on 2026-07-17 — we had been documenting `0.20.4` while running `0.24.0`, 8 versions behind). `OLLAMA_NUM_PARALLEL=2`, `OLLAMA_KEEP_ALIVE=-1`, `OLLAMA_MAX_LOADED_MODELS=10` (in `~/.zshrc` **and** `launchctl setenv` — Ollama is the Mac app launched by launchd, so it reads launchctl; **`brew upgrade ollama` does nothing** — a stale brew formula sits at 0.16.3). **Include the Ollama version in soak checks** (`curl -s localhost:11434/api/version`) so this can't drift silently again. 0.32.1 brought glm 13.7→77.8 tok/s, gpt-oss 50.9→74.5, and working prefix caching — all via llama.cpp; Ollama's native MLX runner exists but is **not enabled** (`OLLAMA_NEW_ENGINE` unset). See `docs/plans/ollama-0.32-upgrade-and-mlx-evaluation.md`.
