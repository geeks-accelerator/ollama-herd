...
uv run pytest                    # run all 977 tests (~40s)
...
- **Trace DB write failures are an observability black hole if you only look at the dashboard.** `record_trace` is fire-and-forget — when `latency.db` writes start failing (`database is locked` under WAL contention), requests still succeed end-to-end but the dashboard's `reqs_24h` drops to 0 because it queries the same DB that's not getting writes. Since 0.6.2 there's a `trace_store_write_failures` health check that surfaces this directly, and the retry-on-locked loop in `record_trace` (3 attempts, 200ms→2s backoff) absorbs short contention silently. If the check fires, the most common root causes are: a long-running read holding the WAL checkpoint, disk-full on the data dir, or a stale `latency.db-shm/-wal` from a crashed previous process. See 2026-05-15 observation.
