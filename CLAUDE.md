...
- **Version:** `0.9.0` in `pyproject.toml`, running on the local fleet (soaking); **not yet published** to PyPI/Homebrew — the latest published release is still **`0.7.0`**. `0.8.0`/`0.8.1`/`0.8.2` were dev milestones that were never published, so **0.9.0 ships all of them together**. Versioned `0.9.0` rather than `0.8.2` because the accumulated diff from `0.7.0` is **breaking**: legacy single-server MLX env vars removed, `X-Fleet-Model` retired for `X-Fleet-Served-Model`, queue-full `503`→`429`, pins now refusable (`409`), image requests now fail rather than silently falling back to a blind model, and backend `4xx` surfaces as `4xx` instead of `500`. Headline features since 0.7.0: distributed multi-Mac MLX, the `/fleet/limits` + `/fleet/pin` control API, `mlx:` models over the OpenAI endpoint, canonical `X-Fleet-*` headers, per-request strict mode, per-client concurrency cap, **optional Anthropic model map (best-loaded auto-routing)**. See `CHANGELOG.md` → `[0.9.0]` for the upgrade guide.
  - **Publish coupling (do this in the 0.9.0 publish pass):** the marketing site (`ollamaherd.com`, separate private repo) documents the *released* version. Its live pages still describe **0.7.0**, which ships the **populated** default map — so the auto-routing / optional-map copy and the `anthropic_no_chat_model`+`anthropic_unrouted_config` health-check + `36→38` changelog note are **staged there, gated on this PyPI publish**. When 0.9.0 goes to PyPI, tell the site agent to ship the staged copy in the same pass (otherwise the site would document behavior the installed version doesn't have). 0.7.0-correct fixes (e.g. the stale `claude-opus-*` wildcard example) already shipped live.
...
- **Health:** 38 distinct checks (count via `grep -oE 'check_id="[^"]+"' src/fleet_manager/server/health_engine.py | sort -u | wc -l`). Monitor: `curl http://localhost:11435/dashboard/api/health`
