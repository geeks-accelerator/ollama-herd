...
uv run pytest                    # run all 1243 tests (~40s)
...
- **Version:** `0.9.1` in `pyproject.toml`. `0.9.0` published to PyPI + Homebrew on 2026-07-19 (fresh-user `brew install` verified after adding the `brew trust` step). `0.9.1` is a fixes-and-hardening patch — session affinity (8th scoring signal), `decode_degraded` (#39) + `pin_cannot_fit` (#40) health checks, queue concurrency capped at the backend's real admission limit, and three bug fixes (`FLEET_NODE_NODE_ID` shadowing, image content-part detection, dashboard SSE wedge). No breaking changes, no new dependencies — so the Homebrew bump is version + sha256 only, no new `resource` blocks. See `CHANGELOG.md` → `[0.9.1]`.
