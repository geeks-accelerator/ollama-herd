...
uv run pytest                    # run all 1344 tests (~15s)
...
- **Throughput incident, resolved 2026-08-23:** fleet decode on `gpt-oss:120b` ran ~15% below baseline for two days (mean 69→59 tok/s). **It was not Ollama, llama.cpp, or herd** — a co-located client (the globally-installed `openclaw` CLI daemon, config `~/.openclaw/openclaw.json`) was calling `http://127.0.0.1:11434` directly, contributing **27% of Ollama's load that herd could not see**, including 77K-token re-prefills every 30 minutes. It arrived there by *fallback*, not by configuration: its cloud provider lost its API key and its model-fallback chain silently rerouted onto the local fleet. Resolved by stopping the daemon and disabling its launch agent (`launchctl bootout gui/$UID/ai.openclaw.gateway` + `launchctl disable`) — the project was no longer in use. Post-removal, p25 returned to exactly the healthy baseline (41.7 → 74.2) and the concurrency cliff vanished (conc=3: ~50 → 74.6). **Note `gemma3:27b` now loads at `CONTEXT 32768` as configured — the `FLEET_NUM_CTX_OVERRIDES` "bug" filed during triage was the bypassing client, not herd.** See the co-tenancy gotcha above, `docs/observations.md` (2026-08-23), and `docs/troubleshooting.md`.
...
- **Version:** `0.9.4` in `pyproject.toml` (unreleased — leaderboard disclosure; `0.9.4` is the version the site's pseudonymous-listing gate keys on, so it must actually ship before listing is enabled: `docs/plans/pseudonymous-leaderboard.md`). `0.9.3` is the current PyPI release. `0.9.0` published to PyPI + Homebrew on 2026-07-19 (fresh-user `brew install` verified after adding the `brew trust` step). `0.9.1` is a fixes-and-hardening patch — session affinity (8th scoring signal), `decode_degraded` (#39) + `pin_cannot_fit` (#40) health checks, queue concurrency capped at the backend's real admission limit, and three bug fixes (`FLEET_NODE_NODE_ID` shadowing, image content-part detection, dashboard SSE wedge). No breaking changes, no new dependencies — so the Homebrew bump is version + sha256 only, no new `resource` blocks. See `CHANGELOG.md` → `[0.9.1]`.
  - **Publish coupling:** the marketing site (`ollamaherd.com`, separate private repo) documents the *released* version. When `0.9.0` published, its `0.9.0` copy was cleared to ship. If the site still lags, the pending items are the `0.9.0` headline features (Codex `/v1/responses`, optional Anthropic model map / best-loaded auto-routing) and the "8-Signal" / 40-checks / test-count copy — tell the site agent to reconcile against the shipped release.
