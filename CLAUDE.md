...
| `server/health_engine.py` | Fleet health analysis: 13 checks (offline, degraded, memory pressure, underutilized, VRAM fallbacks, thrashing, timeouts, error rates, retries, client disconnects, incomplete streams, version mismatch, context protection, zombie reaper) |
...
If it requires a manual, it's too complex. Two CLI commands (`herd`, `herd-node`), zero config files, zero Docker, zero Kubernetes. 436 tests run in under 5 seconds. The entire codebase fits in one person's head. Every time there's a choice between a "proper" distributed systems solution (service mesh, etcd, gRPC) and the simple thing (HTTP heartbeats, SQLite, mDNS) — choose the simple thing. Kill complexity before it kills you.
