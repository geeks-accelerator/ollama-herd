...
Image requests flow through the same queue manager as LLM requests. Open `http://localhost:11435/dashboard` and you'll see image queues in the **Node Models** section alongside LLM, MLX, and embedding model cards — with pending, in-flight, done, and failed counters.
