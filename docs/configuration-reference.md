...
| `FLEET_NODE_MLX_DRAFT_MODEL` | `""` | Speculative-decoding draft model (HF repo id or local path). Must share the main model's tokenizer family — e.g. `mlx-community/Qwen3-1.7B-4bit` for Qwen3-main. Empty disables. **Architecture-specific compatibility (verified 2026-04-27 on mlx-lm 0.31.3):** ✅ works on standard transformer MoEs (e.g. `Qwen3-Coder-30B-A3B-Instruct-4bit` — ~94 tok/s on M3 Ultra). ❌ still fails on hybrid linear-attn MoEs (e.g. `Qwen3-Coder-Next-4bit`) which build a non-trimmable `ArraysCache` — request fails with `ValueError: Speculative decoding requires a trimmable prompt cache`. The "always blocked" framing was wrong: `ArraysCache.is_trimmable()` correctly returns False for SSM-style state; mlx-lm's spec-decode path needs rollback support that linear-attn caches genuinely can't provide. Tracking [mlx-lm#1081](https://github.com/ml-explore/mlx-lm/issues/1081). When using `FLEET_NODE_MLX_SERVERS`, set `draft_model` per-entry instead of via this single-server var. See `docs/issues/mlx-speculative-decoding-blocked.md`. |
