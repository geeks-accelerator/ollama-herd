...
**DeepSeek-V4-Flash — arch not in the runtime.** `mlx-community/DeepSeek-V4-Flash-4bit` downloads fine but `mlx_lm.generate` fails with `Model type deepseek_v4 not supported`. `deepseek_v4` is in *no* released mlx-lm and not on git main either (only an unmerged PR); the community runs it by hand-dropping a `deepseek_v4.py` into `mlx_lm/models/` — a monkeypatch, not a supported install — and even then it has a decode-time Metal leak (mlx-lm #1332, crash at ~11K tokens). Not deployable.

**GLM-5-4bit — too big to *load*, though it fits on disk.** The `mlx-community/GLM-5-4bit` build is **419 GB** (744B params). On the first inference (lazy load), RSS climbs to ~290 GB then the process is **SIGKILLed with no traceback** — even on an empty box with ~510 GB free and `iogpu.wired_limit_mb` raised to 448 GB. Root cause: **MLX's mmap load transient** — `load_safetensors` maps the file and the first forward pass forces those pages resident *while* copying them into Metal buffers, so peak ≈ ~2× the loaded portion, crossing physical RAM → macOS jetsam kills it. **Raising the wired limit does NOT help** — the trigger is total physical RAM (wired buffers + not-yet-evicted mmap pages), not the wired cap. A model's on-disk size fitting in RAM says nothing about whether it *loads*.

**Forcing it crashed the box.** With `iogpu.wired_limit_mb` raised to 448 GB (leaving only 64 GB for the OS) + a 279 GB HF download flooding the file cache + the bots running, the machine **hard-locked and force-reset** (`shutdownStall` + `ResetCounter`, no `.panic` file, and the pre-crash unified log did not survive — the signature of a total lockup, not a graceful panic). Cause is **undeterminable** from the surviving logs; most consistent with a GPU/Metal or memory-pressure deadlock under the combined heavy-MLX + wired-raise + disk-cache load. Notably only ~110 GB was actually wired at crash time, so simple wired-exhaustion doesn't fully explain it — the honest verdict is *unknown*, and heavy MLX work on this box can lock it up without a trace.

**Insight:** On 512 GB Apple Silicon, **runtime maturity and the mmap load ceiling gate model availability far more than RAM quantity or benchmark score** — verify a model *loads and generates*, don't infer it from disk size or a leaderboard. Smaller GLM-5.2 quants (256–360 GB, isolated venv + the PR #1410 `glm_moe_dsa.py`) are the theoretical path, but the DeepSeek-sparse-attention decode path *also* crashes at ~6.5K tokens on this exact hardware (mlx-lm #656), so even a loadable build is decode-unstable. **`iogpu.wired_limit_mb` is a footgun** — raising it to fit an oversized model removes the OS's memory safety margin and can hard-lock the machine; the reward (a decode-unstable model) isn't worth risking a serving box. **What we run instead:** GLM-4.7-Flash and GLM-4.7 full (~180 GB) co-reside cleanly with the bots. See also the 2026-07-17 "I kernel-panicked the box by ignoring a number" entry — same family of lesson: respect the memory numbers.

**What actually shipped from this evaluation:** four new *runnable* models tested and added to the catalog (`qwen3-vl:32b` vision, `qwen3.6:27b`, `qwen3.6:35b-a3b`, `muse-glimmer:30b`), the Ollama upgrade that unlocked muse-glimmer, and the `is_thinking_model` fix so muse-glimmer's `thinking` budget gets inflated. The big two were correctly *rejected* — a real outcome, not a failure.

---

## 2026-07-31: A DHCP lease change orphaned the co-located node — mDNS cached a dead router IP

**Evidence:** A client (`Sable`) hammered the router with `Ollama 503 ({"error":"model 'gpt-oss:120b' exists but no node can serve it right now"})`, retrying every 10s. Everything looked healthy locally: `herd`, `herd-node`, and Ollama were all up, `gpt-oss:120b` + `gemma3:27b` + `deepseek-r1:70b` resident, both MLX servers healthy. But `/fleet/status` showed **both nodes offline**, and the node log was repeating `WARNING Cannot reach router at http://10.0.0.224:11435: All connection attempts failed`. The machine's current IP was **10.0.0.100** (en1); `10.0.0.224` was a dead address — the box's *previous* DHCP lease. The node had discovered the router via mDNS back when the router advertised `.224`, cached it, and never refreshed after the lease changed. Router alive, node alive, zero online nodes, every request 503s. Fix: pinned `FLEET_NODE_ROUTER_URL=http://localhost:11435` in `~/.fleet-manager/env` and clean-restarted both — `bb` re-registered online and a live `gpt-oss:120b` generate returned 200.

**Insight:** This is the **router-URL sibling of the 2026-07-30 node-id drift** — same root cause (a network-derived value that DHCP/roaming silently mutates), different field. Pinning `FLEET_NODE_NODE_ID=bb` fixed identity but did nothing for *discovery*: mDNS still resolved the router to a stale LAN IP. On a box that runs **both** the router and a node, the node should never route through the LAN to reach its own local router — pin `FLEET_NODE_ROUTER_URL=http://localhost:11435` and mDNS drops out of the loop entirely, DHCP-proof. (Only effective since `0.9.1` fixed the kwarg-shadowing bug that made this exact env var inert.) General rule for this fleet: **any value derived from the network — hostname, LAN IP, mDNS advertisement — will drift the moment you change networks; pin the ones that anchor identity or discovery.** The tell was in the first two commands: node process up + `Cannot reach router at <an-IP-that-isn't-ours>` = stale discovery, not a crash.
...
## 2026-07-18 — Codex's tool descriptions document an API its tool schema doesn't expose

**Evidence**: Two independent Codex failures, same root cause, found within an hour of each other while driving `codex-cli 0.145.0-alpha.18` against local models.

1. **`exec_command` called as a top-level tool.** In code mode the only callable tool is `exec`; everything else lives behind `await tools.exec_command(…)` inside the JavaScript it evaluates. The `exec` description is ~14 KB and dense with `tools.exec_command(...)`. Measured on gpt-oss:120b: **4/4** calls went to `exec_command`, a tool never offered — identically with and without our own guidance text, so this is the model reading Codex's prose. Codex gets a `function_call` it has no handler for and **the run stops with nothing displayed** — the transcript shows a tool call, then silence, then the user typing "continue".
2. **`apply_patch` called as a tool.** It isn't one. It's a binary Codex injects at the front of the sandbox PATH — confirmed live by having Codex run `which -a apply_patch` through the working exec path: `/Users/…/.codex/tmp/arg0/codex-arg0MrrzFR/apply_patch`. The instructions say *"Use the `apply_patch` tool to edit files"* while documenting an argv-array shell invocation. Models call it as a tool; Codex answers `error=unsupported call: apply_patch`. The session then reads and executes fine but **can never write** — 40+ turns of tool calls, zero edits.

Both fixed by rewriting at the response boundary: (1) → `custom_tool_call` on `exec` with equivalent JS, (2) → an `exec_command` heredoc. After the fixes, `codex exec` on qwen3-coder:30b ran pytest, read the source, patched it, re-ran pytest and reached `2 passed` — and added an empty-sequence guard nobody asked for.

**Insight**: A client's tool *descriptions* and its tool *schema* can disagree, and frontier models are tuned around the discrepancy while local models are not — they call exactly what the prose names. That makes a whole class of compatibility bug invisible from the server: every metric says success (stream completed, tokens generated, tool call emitted) because the request *did* succeed; only the agentic loop knows it received something unrunnable. Two practical consequences. First, **the client's error channel is where these show up** — `error=unsupported call: apply_patch` appeared only in Codex's stderr, never in ours, and finding it required driving the real client rather than replaying captures. Second, a shim's job includes bridging *documented-but-unoffered* names, and the safe form of that is narrow: rewrite only when the name genuinely wasn't offered and the payload is unambiguous, and pass through untouched otherwise — an empty known-tool set means "we don't know what was offered", not "nothing was offered", and getting that backwards turns legitimate calls into garbage.

**Corollary on method**: single-turn synthetic probes did not reproduce either bug — in a fresh context the model calls `exec_command` correctly, and the `apply_patch` call only emerges mid-conversation. Both were only visible under a real multi-turn client session. Captures are good for pinning schemas; they are not a substitute for running the actual client.

---

## 2026-07-18 — Codex's `/v1/models` schema is an undocumented decode chain, and a single probe lies about it

**Evidence**: Tool-coverage testing against a real `codex-cli 0.145.0-alpha.18` surfaced `failed to refresh available models: … failed to decode models response`. Codex decodes `/v1/models` against its own strictly-typed schema and fails the **entire** decode on the first problem, so each fix reveals exactly one more requirement. Walked in this order: `visibility: "public"` → *unknown variant, expected `list`|`hide`|`none`*; then missing `supported_in_api`, `priority`, `base_instructions`, `support_verbosity`; then `truncation_policy`, which turned out to be a **struct** (`invalid type: string, expected struct TruncationPolicyConfig`), needing `mode` (*expected `bytes` or `tokens`*) and `limit`; then `supports_parallel_tool_calls`; then `experimental_supported_tools`. Eight rounds in, still not converged.

**Insight, and the expensive part**: **a single probe after a restart reports a false clean.** The models refresh hasn't fired yet, so the log is empty and it looks solved — this produced three separate false "CLEAN" results, each of which would have been committed as done. A *dead server* produces the same false clean for a different reason: no response means no decode error, so `decode_err=0` can mean "fixed" or "nothing was listening". The reliable check is **three consecutive runs plus an explicit liveness assertion** (`curl -o /dev/null -w '%{http_code}'` on the endpoint in the same breath). Generalises past this endpoint: when a check's negative result and its "no data" result are indistinguishable, the check is not evidence until you separately prove the system was up and exercised.

**Practical stance**: the field set is unbounded and undocumented, so treat completing it as maintenance, not a milestone. It is genuinely cosmetic for the CLI (`-m` bypasses the picker) but is what empties the Desktop app's model picker — and an empty picker forces Desktop onto its built-in `sol`/`luna`/`terra` Lite slugs, which is a real functional difference, not just a UI blemish. Discovery loop for whoever picks it up: add the field, restart, run `codex exec` **three** times, `grep -o 'missing field `[a-z_]*`'`, repeat. Wrong enum values are useful — the error names the valid variants.

---

## 2026-07-18 — A dropped image doesn't fail, it hallucinates

**Evidence**: Codex's `view_image` tool "worked" — the call round-tripped, Codex was happy, and `qwen3-coder:30b` confidently reported a **32×32** red PNG as *"a 1x1 pixel PNG with a red color value of #FF0000"*. Both details were invented. Asked explicitly to say plainly if it couldn't see the image, it invented dimensions again rather than admitting it.

Two independent causes, both ours. `_text_from_content` in `responses_translator` extracted only text parts, so an `input_image` was **silently discarded** in translation — the model received a question about a picture it never got, with nothing signalling the absence. And `resolve_model` has accepted a `has_images` flag all along (the Claude Code route passes it, `model_knowledge.is_vision_model` + `rank_candidates` implement it), but the Responses route called it without the argument, so it defaulted to `False` and image requests routed to whatever coding model was loaded.
